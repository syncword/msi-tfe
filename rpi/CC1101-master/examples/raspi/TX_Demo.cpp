...
uint8_t cc1100_debug = 1;								//set CC1100 lib in no-debug output mode
uint8_t tx_retries = 1;
// uint8_t rx_demo_addr = 3;
uint8_t rx_demo_addr = BROADCAST_ADDRESS;
int interval = 1000;
...
int main(int argc, char *argv[]) {
	...
	//cc1100.set_mode(0x06);                //set modulation mode 1 = GFSK_1_2_kb; 2 = GFSK_38_4_kb; 3 = GFSK_100_kb; 4 = MSK_250_kb; 5 = MSK_500_kb; 6 = OOK_4_8_kb
	//cc1100.set_ISM(0x02);                 //set ISM Band 1=315MHz; 2=433MHz; 3=868MHz; 4=915MHz
	//cc1100.set_channel(0x01);             //set channel
	cc1100.set_output_power_level(PA_LowPower);       //set PA level
	...
	for (;;)
	{
		...
		if (millis() - prev_millis_1s_timer >= interval)    // one second update timer
		{
				...
				uint32_t time_stamp = 2930683566;
